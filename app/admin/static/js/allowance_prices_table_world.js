$(document).ready(function () {
    ...
    $('.btn-edit-world').click(function () {
        ...
        $row.find('.btn-edit-world').hide();
        $row.find('.edit-mode-world').show();

    });

    $('.btn-cancel-world').click(function () {
        let $row = $(this).closest('tr');
        ...
        $('.btn-edit-world').prop('disabled', false).show();
    });

    $('.btn-save-world').click(function () {
        let $row = $(this).closest('tr');
        let id = $row.data('id');
        let allowance = $row.find('.view-mode-world:eq(1) input').val();
        let note = $row.find('.view-mode-world:eq(2) input').val();

        fetch('/admin_menu/edit_price_world', {
            method: 'POST',
            headers: {
