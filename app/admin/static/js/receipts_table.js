$(document).ready(function () {
    let table = $('#receiptsTable').DataTable({
        "ordering": false,
        "paging": false,
        "info": false,
    ...
    function updateMoveButtons() {
        let rows = $('#receiptsTable tbody tr');
    ...
    $('.btn-edit').click(function () {
        ...
        $row.find('.btn-show-receipt').prop('disabled', false).show();
    ...
    $('.btn-cancel').click(function () {
        ...
        $('.btn-show-receipt').prop('disabled', true);
    });
    ...
    $('.btn-save').click(function () {
        ...
        let id = $row.data('id');
        let name = $row.find('input').val();
        let is_show_receipt = $row.find('.btn-show-receipt').hasClass('btn-success') ? 1 : 0;
        let order = $row.data('order');

        fetch('/admin_menu/edit_receipt', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({ id: id, name: name, is_show_receipt: is_show_receipt, order: order })
        })
    ...
    $('.btn-delete').click(function () {
        let $row = $(this).closest('tr');
        let receipt = $row.find('.receipt-cell').text().trim();
        receiptToDelete = $row.data('id');

        $('#receiptToDelete').text(receipt);
        $('#deleteConfirmationModal').modal('show');
    });

    $('#confirmDeleteBtn').click(function () {
        if (receiptToDelete) {
            fetch('/admin_menu/delete_receipt', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                ...
                .then(response => {
                    if (response.success) {
                        $('tr[data-id="' + receiptToDelete + '"]').remove();
    ...
    $('.btn-show-receipt').click(function () {
        let $button = $(this);
    ...
    $('#btn-add').click(function () {
        ...
        $('#receiptsTable tbody').append($newRow);
        feather.replace();
    ...
    $(document).on('click', '.btn-save-new', function () {
        let $row = $(this).closest('tr');
        let receiptName = $row.find('input').val();

        fetch('/admin_menu/add_receipt', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({ receiptName: receiptName })
        })
            .then(response => response.json())
            .then(response => {
                if (response.success) {
                    $row.attr('data-id', response.id);
                    $row.find('.view-mode').text(receiptName);
                    $row.find('.edit-mode').hide();
                    $row.find('.btn-edit').show();
    ...
    function updateOrder(id, newOrder) {
        fetch('/admin_menu/update_receipts_order', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
