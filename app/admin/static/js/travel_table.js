$(document).ready(function () {
    let table = $('#travelTable').DataTable({
        "ordering": false,
        "paging": false,
        "info": false,
    ...
    $('.btn-edit').click(function () {
        ...
        $row.find('.btn-show-travel').prop('disabled', false).show();
    ...
    $('.btn-cancel').click(function () {
        ...
        $('.btn-show-travel').prop('disabled', true);
    });
    ...
    $('.btn-save').click(function () {
        ...
        let id = $row.data('id');
        let name = $row.find('input').val();
        let is_show_travel = $row.find('.btn-show-travel').hasClass('btn-success') ? 1 : 0;
        ...
        fetch('/admin_menu/edit_travel', {
            ...
            body: JSON.stringify({ id: id, name: name, is_show_travel: is_show_travel, order: order })
        })
    ...
    $('.btn-delete').click(function () {
        let $row = $(this).closest('tr');
        let travel = $row.find('.travel-cell').text().trim();
        travelToDelete = $row.data('id');

        $('#travelToDelete').text(travel);
    ...
    $('#confirmDeleteBtn').click(function () {
        if (travelToDelete) {
            fetch('/admin_menu/delete_travel', {
                ...
                body: JSON.stringify({ id: travelToDelete })
            })
                .then(response => response.json())
                .then(response => {
                    if (response.success) {
                        $('tr[data-id="' + travelToDelete + '"]').remove();
    ...
    $('.btn-show-travel').click(function () {
        let $button = $(this);
    ...
    $(document).on('click', '.btn-save-new', function () {
        let $row = $(this).closest('tr');
        let travelName = $row.find('input').val();
        ...
        fetch('/admin_menu/add_travel', {
            ...
            body: JSON.stringify({ travelName: travelName })
        })
            .then(response => response.json())
            .then(response => {
                if (response.success) {
                    $row.attr('data-id', response.id);
                    $row.find('.view-mode').text(travelName);
                    $row.find('.edit-mode').hide();
                    $row.find('.btn-edit').show();
    ...
    function updateOrder(id, newOrder) {
        fetch('/admin_menu/update_single_order', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
